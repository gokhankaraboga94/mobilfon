...
function toggleSayimModu() {
    console.log(`🔘 toggleSayimModu() ÇAĞRILDI - Mevcut durum: ${sayimModuActive ? 'AÇIK' : 'KAPALI'}`);
    ...
    if (sayimModuActive) {
        ...
        // ⭐ ÖNEMLİ: Önce tüm overlay'leri kapat
        // Bu, overlay input'larının Sayım Modu kapatıldıktan sonra tetiklenmesini engeller
        console.log("🚪 Tüm overlay'ler kapatılıyor...");

        closeDashboardSection();

        // Kısa bir gecikme ile overlay'lerin kapanmasını bekle
        setTimeout(async () => {
            // Okutulmayan barkodları gri listeye transfer et
...
function initializeSayimModuCache() {
    ...
    sayimModuSelectedLists.forEach(listName => {
        if (userCodes[listName] && userCodes[listName].size > 0) {
            ...
            console.log(`  ✅ ${listName}: ${userCodes[listName].size} cihaz cache'lendi`);
        }
    });
...
/**
 * Sayım Modu kapatıldığında okutulmayan barkodları gri listeye transfer et
 */
async function processSayimModuDeactivation() {
    ...
    // Okutulmayan barkodları bul (cache'de var ama scanned'de yok)
    const unscannedBarcodes = Array.from(sayimModuCache).filter(
        barcode => !sayimModuScannedBarcodes.has(barcode)
    );

    console.log(`❌ Okutulmayan barkodlar (${unscannedBarcodes.length} adet):`, unscannedBarcodes);

    // Sadece 15 haneli barkodları filtrele
    const unscanned15Digit = unscannedBarcodes.filter(barcode => /^\d{15}$/.test(barcode));

    console.log(`🔍 Sayım Modu Kapanıyor:`);
    console.log(`   Toplam cache: ${sayimModuCache.size}`);
    console.log(`   Okutulan: ${sayimModuScannedBarcodes.size}`);
    console.log(`   Okutulmayan: ${unscannedBarcodes.length}`);
    console.log(`   Okutulmayan 15 haneli: ${unscanned15Digit.length}`);

    if (unscanned15Digit.length > 0) {
        showToast(`🔄 ${unscanned15Digit.length} adet okutulmayan barkod gri listeye aktarılıyor...`, 'info');
        ...
        for (const barcode of unscanned15Digit) {
            try {
                ...
                if (sourceList) {
                    console.log(`🔄 Transfer ediliyor: ${barcode} (${sourceList} → gri liste)`);
                    // Gri listeye ekle - addToGriListe(barcode, fromList, toList, user)
                    // toList = sourceList çünkü onaylandığında aynı listeye geri dönecek
                    const success = await addToGriListe(barcode, sourceList, sourceList, currentUserName);
                    if (success) {
                        successCount++;
                        console.log(`✅ Transfer başarılı: ${barcode}`);
                    } else {
                        failCount++;
                ...
                // Her 10 barkodda bir kısa bekleme (Firebase rate limit)
                if ((successCount + failCount) % 10 === 0) {
                    await new Promise(resolve => setTimeout(resolve, 100));
                }

            } catch (error) {
                console.error(`❌ ${barcode} gri listeye eklenirken hata:`, error);
                failCount++;
            }
        }

        showToast(
            `✅ Sayım Modu KAPANDI!\n${successCount} barkod gri listeye eklendi${failCount > 0 ? `, ${failCount} hata` : ''}`,
            successCount > 0 ? 'success' : 'warning'
        );

        console.log(`✅ Sayım Modu kapatıldı - ${successCount} başarılı, ${failCount} hata`);
    } else {
        showToast('✅ Sayım Modu KAPANDI! Tüm cihazlar okutuldu.', 'success');
        console.log('✅ Sayım Modu kapatıldı - Tüm cihazlar okutuldu');
    }
...
/**
 * Barkodun cache'de olup olmadığını kontrol et
 */
    ...
    clonedInput.addEventListener('input', async function (e) {
        ...
        inputTimeout = setTimeout(async () => {
            ...
            if (newIMEIs.length > 0) {
                ...
                console.log(`📊 Sayım Modu durumu: ${sayimModuActive ? 'AÇIK' : 'KAPALI'}`);
...
function showMainView() {
    ...
    try {
        ...
        // ✅ SAYIM MODU BUTONU - SADECE ADMIN
        const sayimModuBtn = document.getElementById('sayimModuBtn');
        if (sayimModuBtn) {
            if (currentUserRole === 'admin') {
                sayimModuBtn.style.display = 'block';
            } else {
                sayimModuBtn.style.display = 'none';
            }
        }
...
async function loadPrimValues() {
    try {
        ...
        const parcaKeys = [
            ...
            'cipAktarmaParca', 'ekran', 'kasa', 'kameraCami', 'arkaKapak',
        ...
        const hizmetKeys = [
            'cipAktarmaHizmet', 'yukseltme', 'camSilme', 'yazilimYukleme', 'swapKarari',
            ...
            if (order.timestamp >= startOfPeriod && order.timestamp <= endOfPeriod) {
                ...
                if (order.parts && Array.isArray(order.parts)) {
                    order.parts.forEach(part => {
                        // DEBUG: Parça adını logla
                        if (technician.toLowerCase() === 'ismail' && part.name.toLowerCase().includes('chip')) {
                            console.log('🔍 İSMAİL - ÇİP BULUNDU:', part.name);
                        }
                        
                        // calculatePartPoints fonksiyonunu kullan
                        const points = calculatePartPoints(part.name, primValues);
                        
                        // DEBUG: Puan hesaplamasını logla
                        if (technician.toLowerCase() === 'ismail' && part.name.toLowerCase().includes('chip')) {
                            console.log('   → Hesaplanan puan:', points);
                        }
                        
                        if (points > 0) {
                            // Hangi alana eşleştiğini bul
                            const matchedKey = findMatchingKey(part.name, primValues);
                            
                            // DEBUG: Eşleşen anahtarı logla
                            if (technician.toLowerCase() === 'ismail' && part.name.toLowerCase().includes('chip')) {
                                console.log('   → Eşleşen anahtar:', matchedKey);
                            }
                            
                            if (matchedKey) {
                                technicianScores[technician] += points;
                                technicianDetails[technician][matchedKey] = (technicianDetails[technician][matchedKey] || 0) + 1;
                            }
                        }
                    });
                }
...
// Parça adına göre eşleşen primValues anahtarını bul
function findMatchingKey(partName, primValues) {
    // ÖNCE Türkçe karakterleri değiştir, SONRA lowercase yap
    const partNameLower = partName
        .replace(/İ/g, 'i')
        .replace(/I/g, 'i')
        .replace(/Ş/g, 's')
        .replace(/ş/g, 's')
        ...
        .replace(/ı/g, 'i')
        .toLowerCase()
        .replace(/\s+/g, '');  // TUM BOSLUK, TAB, NEWLINE KARAKTERLERINI KALDIR
    ...
    const checks = {
        ...
        'cipAktarmaHizmet': ['chipaktarim', 'chipaktarma', 'cipaktarim', 'cipaktarma', 'chip', 'cip', 'nand', 'ic'],
        'cipAktarmaParca': ['chipaktarim', 'chipaktarma', 'cipaktarim', 'cipaktarma', 'chip', 'cip', 'nand', 'ic'],
    ...
    for (const [key, value] of Object.entries(primValues)) {
        ...
        // Direkt eşleşme kontrolü (yedek)
        if (partNameLower.includes(keyLower)) {
            return key;
        }
    }
...
async function calculateAndDisplayTechnicianScore(technicianName) {
    ...
    try {
        ...
        // 5 dakikada bir güncelle (300000 ms = 5 dakika) - Sayfa yenilendiğinde güncellensin
        const CACHE_DURATION = 300000; // 5 dakika
        
        if (lastCalculation && (now - parseInt(lastCalculation)) < CACHE_DURATION) {
            ...
            if (cachedLastMonth !== null && cachedCurrentMonth !== null) {
                ...
                // Arka planda sessizce güncelle (kullanıcı görmez)
                if ((now - parseInt(lastCalculation)) > CACHE_DURATION * 0.8) {
                    // Cache süresi dolmak üzereyse, arka planda yenile
                    calculateTechnicianScoresBackground(technicianName);
                }
                return;
            }
        }
        
        // Yeni hesaplama yap (loading göster)
...
// Arka planda sessiz güncelleme (kullanıcı fark etmez)
async function calculateTechnicianScoresBackground(technicianName) {
    try {
        const scores = await calculateTechnicianScores(technicianName);
        
        // Sessizce cache'i güncelle
        localStorage.setItem(`techScoreLastMonth_${technicianName}`, scores.lastMonth.toString());
        localStorage.setItem(`techScoreCurrentMonth_${technicianName}`, scores.currentMonth.toString());
        localStorage.setItem(`lastScoreCalc_${technicianName}`, Date.now().toString());
        
        // Ekranı güncelle
        displayTechnicianScores(scores.lastMonth, scores.currentMonth);
    } catch (error) {
        // Sessizce hata yut, kullanıcıyı rahatsız etme
        console.log('Background update failed, will retry on next page load');
    }
}

// Hem geçen ay hem bu ay puanlarını hesapla - OPTİMİZE EDİLMİŞ VERSİYON
async function calculateTechnicianScores(technicianName) {
    try {
        ...
        // Puan değerlerini cache'den al veya yükle
        let primValues = null;
        const cachedPrimValues = localStorage.getItem('cachedPrimValues');
        const cachedPrimValuesTime = localStorage.getItem('cachedPrimValuesTime');
        
        // Prim değerleri 1 saat cache'lenir
        if (cachedPrimValues && cachedPrimValuesTime && (Date.now() - parseInt(cachedPrimValuesTime) < 3600000)) {
            primValues = JSON.parse(cachedPrimValues);
        } else {
            const valuesSnapshot = await db.ref('primSettings/values').once('value');
            primValues = valuesSnapshot.val() || defaultPrimValues;
            localStorage.setItem('cachedPrimValues', JSON.stringify(primValues));
            localStorage.setItem('cachedPrimValuesTime', Date.now().toString());
        }
        
        // SADECE bu teknisyenin siparişlerini ve sadece son 2 ayı çek
        const partOrdersQuery = db.ref('partOrders')
            .orderByChild('timestamp')
            .startAt(lastMonthStart.getTime())
            .endAt(currentMonthEnd.getTime());
        
        const partOrdersSnapshot = await partOrdersQuery.once('value');
        const partOrdersData = partOrdersSnapshot.val();
        ...
        // Sadece bu teknisyenin siparişlerini işle
        for (const [orderId, order] of Object.entries(partOrdersData)) {
    ...
    try {
        ...
        if (fromList && fromList !== 'YENİ') {
            ...
            if (userCodes[fromList] && userCodes[fromList].has(barcode)) {
                ...
                if (codeUsers[fromList]) {
                    delete codeUsers[fromList][barcode];
                }
        ...
        // 5. allCodes'a ekle (teslimEdilenler hariç)
    ...
    if (user) {
        ...
        try {
            ...
            if (currentUserRole === 'technician' || allTechnicianUsers.includes(name)) {
                ...
                // ✅ TEKNİSYEN PUANINI HESAPLA VE GÖSTER - OTOMATİK GÜNCELLEME
                // İlk yüklemede hemen hesapla
                calculateAndDisplayTechnicianScore(name);
                
                // Her sayfa yenilendiğinde çalışır (cache süresi 5 dakika)
                // partOrders değiştiğinde de güncelle
                db.ref('partOrders').on('child_added', () => {
                    // Yeni sipariş eklendiğinde cache'i temizle ve yeniden hesapla
                    setTimeout(() => {
                        localStorage.removeItem(`lastScoreCalc_${name}`);
                        calculateAndDisplayTechnicianScore(name);
                    }, 500);
                });
                
                db.ref('partOrders').on('child_changed', () => {
                    // Sipariş değiştiğinde cache'i temizle ve yeniden hesapla
                    setTimeout(() => {
                        localStorage.removeItem(`lastScoreCalc_${name}`);
                        calculateAndDisplayTechnicianScore(name);
                    }, 500);
...
// Çıkış butonu - Sayfa yüklendikten sonra event listener ekle
...
if (navLogoutBtn) {
    navLogoutBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            // Sayfayı yenile - login ekranına dön
    ...
    console.warn('⚠️ navLogoutButton bulunamadı, DOMContentLoaded bekleniyor');
    // Eğer buton henüz yüklenmemişse, DOM yüklendikten sonra tekrar dene
    document.addEventListener('DOMContentLoaded', () => {
        const btn = document.getElementById('navLogoutButton');
        if (btn) {
            btn.addEventListener("click", async () => {
                console.log('🚪 Çıkış butonu tıklandı! (DOMContentLoaded)');
                try {
                    stopConflictMonitoring();
                    await auth.signOut();
                    console.log('✅ Firebase signOut başarılı');
            ...
            console.log('✅ Logout butonu event listener eklendi (DOMContentLoaded)');
        } else {
            console.error('❌ navLogoutButton hala bulunamadı!');
        }
    });
}
        ...
        if (isTechnicianUser()) {
            ...
            allTechnicianUsers.forEach(techName => {
                if (techName !== currentUserName && inputs[techName]) {
                    ...
                    inputs[techName].style.cursor = 'text';
                    inputs[techName].placeholder = `${CACHED_LIST_NAMES[techName] || techName} barkodlarını girin...`;
                }
            });
        }
...
// ========================================
// PART ORDER SYSTEM - MULTI-IMEI SUPPORT
// ========================================

let currentPartOrderType = 'single'; // 'single' or 'multiple'

// Open part order type selection modal
function openPartOrderModal() {
    document.getElementById('partOrderTypeModal').style.display = 'flex';
}

// Close part order type selection modal
function closePartOrderTypeModal() {
    document.getElementById('partOrderTypeModal').style.display = 'none';
}

// Select part order type and open main form
function selectPartOrderType(type) {
    currentPartOrderType = type;
    closePartOrderTypeModal();

    // Open the main part order modal
    document.getElementById('partOrderModal').style.display = 'flex';

    // Configure inputs based on type
    const singleInput = document.getElementById('partOrderBarcode');
    const multipleInput = document.getElementById('partOrderBarcodeMultiple');
    ...
    if (type === 'single') {
        ...
        barcodeCount.style.display = 'none';
        barcodeLabel.textContent = '15 Haneli Barkod:';
        singleInput.value = '';
    } else {
        singleInput.style.display = 'none';
        multipleInput.style.display = 'block';
        barcodeCount.style.display = 'block';
        barcodeLabel.textContent = 'Barkodlar (Çoklu):';
        multipleInput.value = '';
        updateBarcodeCount();
...
function updateBarcodeCount() {
    ...
    if (!multipleInput || !barcodeCount) return;

    const barcodes = parseMultipleBarcodes(multipleInput.value);
    const validCount = barcodes.filter(b => b.length === 15).length;
    const invalidCount = barcodes.length - validCount;

    if (barcodes.length === 0) {
        barcodeCount.textContent = '';
    } else {
        barcodeCount.textContent = `✅ ${validCount} geçerli barkod${invalidCount > 0 ? ` | ❌ ${invalidCount} geçersiz` : ''}`;
        barcodeCount.style.color = invalidCount > 0 ? '#e74c3c' : '#2ecc71';
    }
}

// Parse multiple barcodes from textarea
function parseMultipleBarcodes(text) {
...
// Close part order modal
function closePartOrderModal() {
    document.getElementById('partOrderModal').style.display = 'none';
    currentPartOrderType = 'single';
}
...
// ========================================
// SAYIM MODU SİSTEMİ - MOVED TO TOP OF FILE (LINE ~166)
// ========================================
// toggleSayimModu() fonksiyonu yukarıda tanımlı (satır ~166)
// Bu eski versiyon silindi çünkü processSayimModuDeactivation() çağırmıyordu
